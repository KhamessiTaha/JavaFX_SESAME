    ...
    private static void createTables() {
        try (Statement stmt = connection.createStatement()) {
            ...
            try (Statement migrationStmt = connection.createStatement()) {
                migrationStmt.execute("ALTER TABLE utilisateurs ADD COLUMN status TEXT DEFAULT 'EN_ATTENTE'");
        ...
        try {
            try (Statement stmt = connection.createStatement();
                ...
                if (rs.next() && rs.getInt(1) == 0) {
                    stmt.execute("INSERT INTO utilisateurs (username, password, role, status) VALUES ('admin', 'admin', 'admin', 'ACTIF')");
                    stmt.execute("INSERT INTO utilisateurs (username, password, role, status) VALUES ('agent', 'agent', 'agent', 'ACTIF')");
                }
            }

            try (Statement stmt = connection.createStatement();
                 ResultSet rs = stmt.executeQuery("SELECT COUNT(*) FROM vehicules")) {
                if (rs.next() && rs.getInt(1) == 0) { // Only seed if completely empty
                    stmt.execute("INSERT OR IGNORE INTO vehicules (marque, modele, immatriculation, categorie, carburant, boiteVitesse, nombrePlaces, prixParJour, statut) VALUES " +
            ...
            try (Statement stmt = connection.createStatement();
                ...
                if (rs.next() && rs.getInt(1) == 0) {
                    stmt.execute("INSERT INTO clients (nom, prenom, cin, email, telephone, adresse, numeroPermis, expirationPermis) VALUES " +
            ...
            try (Statement stmt = connection.createStatement();
                ...
                if (rs.next() && rs.getInt(1) == 0) {
                    stmt.execute("INSERT INTO reservations (clientId, vehiculeId, dateDebut, dateFin, nombreJours, prixTotal, statut, options, prixOptions) VALUES " +
                            "(1, 8, '2024-05-01', '2024-05-05', 4, 440.0, 'Terminée', 'GPS', 20.0)," +
                            "(2, 4, '2026-05-10', '2026-05-15', 5, 750.0, 'Confirmée', 'Siège Bébé', 50.0)," +
                            "(3, 1, '2026-06-01', '2026-06-07', 6, 720.0, 'Confirmée', 'GPS, Assurance Plus', 60.0)," +
                            "(4, 2, '2026-05-20', '2026-05-25', 5, 2250.0, 'En attente', 'Aucune', 0.0)," +
                            "(5, 5, '2024-01-15', '2024-01-20', 5, 2400.0, 'Terminée', 'GPS', 30.0)," +
                            "(6, 9, '2024-03-10', '2024-03-12', 2, 360.0, 'Terminée', 'Aucune', 0.0)," +
                            "(7, 12, '2026-07-01', '2026-07-03', 2, 190.0, 'Confirmée', 'Aucune', 0.0)," +
                            "(8, 6, '2026-05-01', '2026-05-10', 9, 1170.0, 'Annulée', 'Siège Bébé', 40.0)," +
                            "(9, 13, '2026-08-15', '2026-08-20', 5, 500.0, 'Confirmée', 'Aucune', 0.0)," +
                            "(10, 15, '2024-02-01', '2024-02-05', 4, 760.0, 'Terminée', 'GPS', 20.0)," +
                            "(11, 10, '2026-09-01', '2026-09-10', 9, 7650.0, 'Confirmée', 'Plein Essence', 100.0)," +
                            "(12, 11, '2026-10-05', '2026-10-08', 3, 660.0, 'Confirmée', 'GPS', 15.0)," +
                            "(1, 7, '2026-11-20', '2026-11-25', 5, 3500.0, 'Confirmée', 'Tout inclus', 200.0)," +
                            "(14, 21, '2024-04-01', '2024-04-05', 4, 2600.0, 'Terminée', 'Aucune', 0.0)," +
                            "(15, 22, '2024-04-10', '2024-04-12', 2, 220.0, 'Terminée', 'Siège Bébé', 20.0)," +
                            "(16, 23, '2026-05-15', '2026-05-20', 5, 3000.0, 'Confirmée', 'Chauffeur', 500.0)," +
                            "(17, 24, '2026-06-10', '2026-06-15', 5, 1400.0, 'En attente', 'GPS', 25.0)," +
                            "(18, 25, '2026-07-01', '2026-07-10', 9, 1440.0, 'Confirmée', 'Aucune', 0.0)," +
                            "(19, 26, '2024-02-15', '2024-02-18', 3, 2850.0, 'Terminée', 'Plein Essence', 100.0)," +
                            "(20, 27, '2026-08-01', '2026-08-03', 2, 210.0, 'Confirmée', 'Aucune', 0.0)," +
                            "(21, 28, '2026-09-10', '2026-09-15', 5, 775.0, 'Confirmée', 'GPS', 20.0)," +
                            "(13, 29, '2026-05-25', '2026-05-30', 5, 2900.0, 'En attente', 'Assurance Plus', 150.0)," +
                            "(3, 30, '2024-03-20', '2024-03-22', 2, 180.0, 'Terminée', 'Aucune', 0.0)");
                }
            }

            try (Statement stmt = connection.createStatement();
                 ResultSet rs = stmt.executeQuery("SELECT COUNT(*) FROM paiements")) {
                if (rs.next() && rs.getInt(1) == 0) {
                    stmt.execute("INSERT INTO paiements (reservationId, montantTotal, montantPaye, resteAPayer, modePaiement, statutPaiement) VALUES " +
                            ...
                            "(25, 180.0, 180.0, 0.0, 'Espèces', 'Payé')");
                }
            }

            try (Statement stmt = connection.createStatement();
                 ResultSet rs = stmt.executeQuery("SELECT COUNT(*) FROM retours")) {
                if (rs.next() && rs.getInt(1) == 0) {
                    stmt.execute("INSERT INTO retours (reservationId, dateRetour, etatVehicule, fraisSupplementaires) VALUES " +
